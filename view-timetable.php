...
<body>
    <div class="container mt-5">
        <div class="d-flex justify-content-between align-items-center mb-3">
           ...
           <!-- Print PDF Link -->
<a href="generate-pdf.php?course=<?php echo urlencode($course); ?>&level=<?php echo urlencode($level); ?>" target="_blank" class="btn btn-success">Print PDF</a>
       ...
       if ($displayTimetable) {
           ...
           while ($entry = $result->fetch_assoc()) {
               ...
               echo "<a href='edit-timetable.php?id={$entry['id']}' class='btn btn-warning'>Edit</a> ";
       
               // Delete button with confirmation prompt
               echo "<button class='btn btn-danger' onclick='confirmDelete({$entry['id']})'>Delete</button>";
       ...
       <script>
           function confirmDelete(id) {
               if (confirm("Are you sure you want to delete this timetable entry?")) {
                   window.location.href = 'delete-timetable.php?id=' + id;
               }
           }
       </script>
