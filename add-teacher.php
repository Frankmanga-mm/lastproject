...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $teacherName = sanitizeInput($_POST["teacher"]);
    $teacherDepartment = sanitizeInput($_POST["department"]);

    // Check for empty fields
    if (empty($teacherName) || empty($teacherDepartment)) {
        $errorMsg = '<div class="alert alert-danger" role="alert">Please fill in all fields.</div>';
    } else {
        // Prepare and bind the statement
        $stmt = $conn->prepare("INSERT INTO teachers (name, department) VALUES (?, ?)");
        $stmt->bind_param("ss", $teacherName, $teacherDepartment);

        // Execute the statement
        if ($stmt->execute()) {
            // Redirect to view teachers after successful addition
