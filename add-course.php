...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $courseName = sanitizeInput($_POST["name"]);
    $courseLevel = sanitizeInput($_POST["level"]);

    // Check for empty fields
    if (empty($courseName) || empty($courseLevel)) {
        $errorMsg = '<div class="alert alert-danger" role="alert">Please fill in all fields.</div>';
    } else {
        // Prepare and bind the statement
        $stmt = $conn->prepare("INSERT INTO courses (name, level) VALUES (?, ?)");
        $stmt->bind_param("ss", $courseName, $courseLevel);

        // Execute the statement
        if ($stmt->execute()) {
            // Redirect to view courses after successful addition
