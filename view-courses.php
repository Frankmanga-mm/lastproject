...
if (isset($_GET['search'])) {
    ...
    $search_condition = "WHERE name LIKE '%$search_query%' OR level LIKE '%$search_query%'";
} else {
    $search_condition = '';
}
...
// Retrieve paginated and filtered courses data
$result = $conn->query("SELECT * FROM courses $search_condition LIMIT $start_index, $results_per_page");
$total_results = $conn->query("SELECT COUNT(*) FROM courses $search_condition")->fetch_row()[0];
    ...
    <div class="container mt-5">
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            <td>{$row['name']}</td>
                            <td>{$row['level']}</td>
                            <td><a href='edit-course.php?id={$row['id']}'>Edit</a></td>
                            <td><a href='delete-course.php?id={$row['id']}' onclick='return confirm(\"Are you sure you want to delete this course?\")'>Delete</a></td>
                        </tr>";
                }
                ?>
            </tbody>
        ...
        <ul class="pagination">
            ...
            for ($page_num = 1; $page_num <= $num_pages; $page_num++) {
                echo "<li class='page-item" . ($page_num == $page ? " active" : "") . "'><a class='page-link' href='view-courses.php?page=$page_num&search=$search_query'>$page_num</a></li>";
            }
            ?>
        </ul>
    </div>
