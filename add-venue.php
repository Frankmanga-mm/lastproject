...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $venueName = sanitizeInput($_POST["venue"]);
    $venueCapacity = sanitizeInput($_POST["quantity"]);

    // Check for empty fields
    if (empty($venueName) || empty($venueCapacity)) {
        $errorMsg = '<span style="color: red;">Please fill in all fields.</span>';
    } else {
        // Prepare and bind the statement
        $stmt = $conn->prepare("INSERT INTO venues (name, capacity) VALUES (?, ?)");
        $stmt->bind_param("ss", $venueName, $venueCapacity);

        // Execute the statement
        if ($stmt->execute()) {
            header("Location: view-venues.php");
        } else {
            $errorMsg = '<span style="color: red;">Error: ' . $stmt->error . '</span>';
        }
    ...
    <div class="container mt-5">
        ...
        <form action="" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="venue" name="quantity">
            </div>
